...
开发者创建 MR
  ...
  → 人工最终确认
```
**提升质量**: 每个 MR 都经过 AI 审查,捕获安全和性能问题
...
sequenceDiagram
    ...
    participant A as AI 执行器
    participant G as Git 仓库

    U->>I: 创建 Issue 并 @claude
    I->>S: 触发 Webhook
    S->>S: 检测 Session 模式
    S->>A: 执行 AI 指令
    A->>G: 创建时间戳分支
...
sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        Note over U,MR: 场景 1: 新建/重新打开的 MR
        U->>MR: 创建 MR 并 @claude
        MR->>S: 触发 Webhook
        S->>A: 执行代码审查
        A->>MR: 发布审查评论
    ...
    rect rgb(255, 250, 240)
        Note over U,MR: 场景 2: MR 评论中的代码修改
        U->>MR: 在评论中 @claude 修改代码
        MR->>S: 触发 Webhook
        S->>A: 执行 AI 指令
        A->>G: 切换到 MR 源分支
        A->>G: 生成代码并提交
